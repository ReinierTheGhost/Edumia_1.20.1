...
import com.legends.edumia.entity.util.variant.EntityVariantList;
...
public class EntityTypeContainer<T extends Mob> {
    ...
    public LazyLoadedValue<EntityType<T>> entityType;
    ...
    protected final LazyLoadedValue<MobSpawnSettings.SpawnerData> spawnEntry = new LazyLoadedValue<>(()
            -> new MobSpawnSettings.SpawnerData(this.entityType.get(), config.spawnWeight.get(), config.spawnMinGroup.get(),
            config.spawnMaxGroup.get()));
    ...
    public static <T extends Mob> boolean waterSpawn(EntityType<T> type, ServerLevelAccessor world, MobSpawnType reason, BlockPos pos,
                                                     RandomSource rand) {
    ...
    public static List<String> setBiomesToIDs(Set<ResourceKey<Biome>> set) {
        return set.parallelStream().filter(Objects::nonNull).map(biomeResourceKey -> biomeResourceKey.location().toString())
   ...
   /* protected void clientCustomConfigurationInit(ConfigBuilder builder) {
    ...
    public static class Builder<T extends Mob> extends AbstractEntityBuilder<T, EntityTypeContainer<T>, Builder<T>> {
        ...
        private Builder(Class<T> EntityClass, EntityType.EntityFactory<T> factory, String entityNameIn, Supplier<AttributeSupplier.Builder>
                attributeMap, String modid) {
